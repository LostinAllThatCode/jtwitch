...
public class MainController implements LivestreamerListener{
			...
			public void run() {
				if (m != null){
					try {
						...
						LogManager.getLogger().error("Json data seem to be corrupt.");
						e.printStackTrace();
					} catch (TwitchAPIUnauthorizedAccessException e) {
						LogManager.getLogger().error("Can't access this resource from twitch api. oAuth2 token is not valid or not enough permissions");
						e.printStackTrace();
					} catch (IOException e) {
						LogManager.getLogger().error("HTTP connection closed. Service not available.");
						e.printStackTrace();
					}
				}
			}
	...
	private synchronized void updateChannelView(ChannelModel m){
		...
		if (m.isOnline()) view.getChannelListView().sortChannels(model.getChannelListModel().getChannels());
	}
				...
				while (true) {
					try {
						if (time / interval >= 4) {
							...
							UserFollowedChannels channels = new TwitchAPI().getResource("https://api.twitch.tv/kraken/users/"+config.getProperty("username")+"/follows/channels",UserFollowedChannels.class);
